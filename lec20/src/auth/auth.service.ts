...
export class AuthService {
  ...
  async signUp({ age, email, fullName, password }: SignUpDto) {
    ...
    const optCodeExpirationDate = new Date().setTime(
  ...
  async resendVerificationCode({ email }: ResendVerificationCodeDto) {
    ...
    if (existUser.isVerfied)
      throw new BadRequestException('You already verified');

    if (new Date().getTime() < Number(existUser.OTPCodeExpirationDate))
      throw new BadRequestException('OTP code is not outdated');
  ...
  async verifyUser({ email, otpCode }: VerifyUserDto) {
    ...
    if (new Date().getTime() > Number(existUser.OTPCodeExpirationDate))
      throw new BadRequestException('OTP code outdated');
